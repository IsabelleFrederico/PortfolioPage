...
export const CatMia = forwardRef(function CatMia(
  { animation = "CatBathing", ...props },
  ref
) {
  const group = useRef()
  ...
  const runningClip = useMemo(() => buildClip(catRunningAnimation, "CatRunning", "Bone"), [catRunningAnimation])
  const standingClip = useMemo(() => buildClip(catStandingAnimation, "CatStanding", "Bone"), [catStandingAnimation])
  const seatedClip = useMemo(() => buildClip(catSeatedAnimation, "CatSeated", "Bone"), [catSeatedAnimation])

  const allAnims = useMemo(() => [bathClip, standingClip, runningClip, seatedClip].filter(Boolean), [
  ...
  useEffect(() => {
    ...
    if (prev.current && prev.current !== next) prev.current.fadeOut(0.2)
    prev.current = next
  }, [animation, actions])
