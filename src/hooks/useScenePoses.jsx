...
export function useScenePoses({ viewport, menuOpened, catAnimation }) {
  ...
  return useMemo(() => {
    const byMenu = (closedPose, openPose) =>
      menuOpened ? openPose : closedPose;
    ...
    const section0MenuClosed = isMobile
      ? [0.1 * officeScaleRatio, -viewH / 6, 0]
      : [0.7, -0.5, 0];
    ...
    const avatarVariants = {
      0: byMenu(
        {
          ...stick0Position(0.15, isMobile ? 0.01 : 0.01, -0.05),
          rotateX: 0,
          rotateY: Math.PI / 2,
          rotateZ: 0,
    ...
    const catVariants = {
      0: byMenu(
        {
          ...stick0Position(-0.1, isMobile ? 0.1 : 0.1, 1),
          rotateX: -Math.PI / 2,
          rotateY: 1,
          rotateZ: 0,
      ...
      3: byMenu(
        {
          ...
          rotateZ: 1,
          scale: isMobile
            ? viewH * runningScaleBoost - 1.3
            : 1.9 * runningScaleBoost,
        },
        {
          x: isMobile ? -0.7 : 3,
    ...
    const officeVariants = byMenu(
      {
        position: [
          isMobile ? 0.1 * officeScaleRatio : 0.7,
          isMobile ? -viewH / 6 : -0.5,
          0,
        ],
      },
      {
        position: [
          isMobile ? 2 * officeScaleRatio : 2.5,
          isMobile ? -0.8 : -0.8,
          1,
        ],
      },
